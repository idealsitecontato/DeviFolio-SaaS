...
function homeView() {
  ...
  const qrMarkup = linkReady
    ? `<div class="dashboard-url"><span data-icon="link"></span><span>${esc(publicPortfolioUrl())}</span><button class="icon-button" data-copy="${esc(publicPortfolioUrl())}" aria-label="Copiar link"><span data-icon="copy"></span></button></div><div class="dashboard-qr"><div class="home-qr-area"><div class="qr-wrap"><canvas id="portfolio-qr" width="150" height="150" aria-label="QR Code do portfólio público"></canvas></div></div><div class="home-qr-download"><button class="secondary-button" data-download-qr><span data-icon="download"></span>Baixar QRCode</button></div></div>`
  ...
  return `<section class="page-enter home-page"><div class="dashboard-layout"><div class="dashboard-main-column"><article class="card welcome-card"><div class="welcome-copy"><p>Bem-vindo de volta,</p><h1>${esc(realName())}!</h1><span>Seu portfólio, projetos e estatísticas, tudo em um só lugar.</span></div><span class="welcome-brand" aria-hidden="true"><img src="assets/devifolio-logo.png" alt=""></span><div class="dashboard-metrics"><button data-route-button="portfolio"><span data-icon="folder"></span><small>Portifolios</small><strong>${state.published ? '1' : '0'}</strong></button><button data-route-button="analise"><span data-icon="eye"></span><small>Visualizações</small><strong>${summary.views || '0'}</strong></button><button data-route-button="analise"><span data-icon="users"></span><small>Seguidores</small><strong>${summary.visitors || '0'}</strong></button><button data-route-button="link-qrcode"><span data-icon="link"></span><small>Links ativos</small><strong>${state.published ? '1' : '0'}</strong></button></div></article><section class="card projects-card"><div class="section-card-head"><h2>Seus projetos recentes</h2>${state.projects.length ? '<button class="link-button" data-route-button="projetos">Ver todos <span data-icon="arrow"></span></button>' : ''}</div>${projectRows(state.projects.slice(0, 4))}</section></div><aside class="dashboard-side-column"><article class="card github-summary"><div class="summary-title"><span data-icon="github"></span><h2>${state.githubConnected ? 'GitHub conectado' : 'Conectar GitHub'}</h2></div>${githubCopy}</article><article class="card link-summary home-qr-card"><h2>Seu Link, Seu QRCode</h2><p>Compartilhe seu portfólio por link ou QR Code.</p>${qrMarkup}</article><article class="card plans-summary"><div class="summary-title"><span data-icon="card"></span><h2>Planos</h2></div><p>Conheça os planos disponíveis para o seu portfólio.</p><button class="dashboard-action" data-route-button="planos">Ver planos<span data-icon="arrow"></span></button></article></aside></div></section>`
}
...
function render() {
  ...
  const routeLabel = { 'link-qrcode': 'Seu link, Seu QRCode', inicio: 'Dashboard', projetos: 'Projetos', portfolio: 'Meus portfólios', 'portfolio-editar': 'Editar portfólio', github: 'GitHub', analise: 'Análise', perfil: 'Perfil', planos: 'Planos', configuracoes: 'Configurações', indicacao: 'Indicação' }[route]
...
async function togglePublished(event) {
  ...
  try {
    const proceed = await runStatusProcess(publishing ? 'Publicando portfólio' : 'Despublicando portfólio', publishing ? ['Preparando projeto...', 'Validando informações...', 'Gerando build...', 'Preparando publicação...', 'Publicando portfólio...'] : ['Preparando despublicação...', 'Removendo versão pública...', 'Atualizando status...', 'Finalizando...'])
    if (!proceed) return
...
async function runStatusProcess(title, steps, { cancellable = false } = {}) {
  ...
  for (const step of $$('[data-process-step]', panel)) {
    if (!panel.isConnected || backdrop.classList.contains('is-closing')) return false
    step.classList.add('is-active')
    await wait(6000 / steps.length)
    if (!panel.isConnected || backdrop.classList.contains('is-closing')) return false
