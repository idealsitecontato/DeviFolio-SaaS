...
function homeView() {
  ...
  const qrMarkup = linkReady
    ? `<div class="dashboard-url"><span data-icon="link"></span><span>${esc(publicPortfolioUrl())}</span><button class="icon-button" data-copy="${esc(publicPortfolioUrl())}" aria-label="Copiar link"><span data-icon="copy"></span></button></div><div class="dashboard-qr"><div class="qr-wrap"><canvas id="portfolio-qr" width="150" height="150" aria-label="QR Code do portfólio público"></canvas></div><button class="secondary-button" data-route-button="link-qrcode">Ver link e QR Code</button></div>`
