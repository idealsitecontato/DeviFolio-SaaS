...
function projectRows(items = state.projects.slice(0, 5)) {
  ...
  return items.map(project => {
    ...
    return `<div class="recent-project"><span class="recent-project-image">${project.image ? `<img src="${esc(project.image)}" alt="">` : '<span data-icon="folder"></span>'}</span><div class="recent-project-copy"><strong>${esc(project.name)}</strong><p>${esc(project.description)}</p></div>${tags.length ? `<div class="recent-project-tags">${tags.map(tag => `<span>${esc(tag)}</span>`).join('')}</div>` : '<div class="recent-project-tags" aria-hidden="true"></div>'}<button class="icon-button" data-view-project="${project.id}" aria-label="Visualizar ${esc(project.name)}"><span data-icon="chevron"></span></button></div>`
  }).join('')
}
